describe('Basic Auth Mode', () => {
  ...
  it('should fail without credentials', () => {
    ...
    cy.setupBasicAuth('invalid', 'invalid');

    // Select basic auth
    cy.get('input[type="radio"][value="basic"]').check();

    // Enter invalid credentials
    cy.get('input[type="text"][placeholder="Username"]').type('invalid');
    cy.get('input[type="password"][placeholder="Password"]').type('invalid');
  ...
  it('should succeed with valid credentials', () => {
    ...
    cy.get('input[type="text"][placeholder="Username"]').type('admin');
    cy.get('input[type="password"][placeholder="Password"]').type('admin');
    ...
    cy.wait('@basicAuthRequest').then((interception) => {
      const authHeader = interception.request.headers['Authorization'];
      const expectedAuth = `Basic ${btoa('admin:admin')}`;
      expect(authHeader).to.equal(expectedAuth);
      expect(interception.response?.statusCode).to.equal(200);
    });
  });

  it('should maintain basic auth across navigation', () => {
    cy.setupBasicAuth();
    ...
    const collectionId = 'basic-test-collection-id';
    const collectionName = 'basic-test-collection';
    ...
    cy.get('[data-test="collection-name-input"]').type('basic-test-collection');
    ...
    cy.get('[data-test="collections-list"]')
      .contains('basic-test-collection')
      .should('be.visible');
    cy.get('div.fixed.inset-0.z-50.overflow-y-auto').should('not.exist');

    // Navigate and verify auth is maintained
    cy.get('[data-test="collection-row"]').first().find('button').first().click();
    cy.url().should('include', '/collection/');

    // All requests should include basic auth
    cy.get('@basicAuthRequest.all').then((interceptions) => {
      const expectedAuth = `Basic ${btoa('admin:admin')}`;
      interceptions.forEach((interception) => {
        expect(interception.request.headers['Authorization']).to.equal(expectedAuth);
      });
    });
