  ...
  <div class="fixed top-4 right-4 z-50 space-y-2">
    <transition-group name="notification" tag="div">
      <div
        ...
        :key="notification.id"
        :class="[
          'rounded-md p-4 shadow-lg transform transition-all duration-300 max-w-[min(calc(100vw-2rem),380px)]',
          {
        ...
        <div class="flex justify-between items-center">
          <div class="flex">
            ...
            <div class="ml-3 flex-grow">
              <p class="text-sm break-words pr-2">{{ notification.message }}</p>
            </div>
          </div>
          <button
            @click.stop="removeNotification(notification.id)"
            class="opacity-70 hover:opacity-100 p-1.5 -mr-1 flex-shrink-0 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors duration-200"
          >
